...
- **Prompt layout aligned with `gliner2`.** `SchemaTransformer` diverged from
  ...
  2. Field indices pointed at the *label name* instead of the `[E]`/`[R]`/`[L]`
     marker that precedes it — an off-by-one. The `[P]` index was already
     correct, so the two were mutually inconsistent.
  3. Text words were not lower-cased, while `_tokenize_text` calls
     ...
     original string, so extracted text preserves its original casing:
     lower-casing the source first is unsafe, because Unicode case folding can
     change its length and corrupt the offsets.

  The model was robust enough to produce usable output regardless, but
  confidences were depressed. On `"Mario Rossi lavora ad Apple a Cupertino e la
  sua email e' mario.rossi@example.com."` against the V2 fine-tuned export, the
  same four entities are returned with `person` going from 99.22% to 100.00%
  and `location` from 98.29% to 99.95%.

  Ground truth is now pinned by `processor::tests::ground_truth_prompt_layout`,
...
- Added `examples/local_check.rs`, a smoke check against a local export.

### Known issues
- The V1 (legacy, CPU-slicing) pipeline still fails on some `count_lstm_fp32`
  exports with `Gather node '/count_lstm/gru/Gather_3': indices element out of
  data bounds`. Pre-existing and unrelated to the above; the V2 IOBinding
